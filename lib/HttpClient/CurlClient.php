...
class CurlClient implements ClientInterface
{
    ...
    public function request($method, $absUrl, $headers, $params)
    {
        ...
        if (is_callable($this->defaultOptions)) { // call defaultOptions callback, set options to return value
            ...
            if (!is_array($opts)) {
                throw new Error\PayhereError("Non-array value returned by defaultOptions CurlClient callback");
            }
        ...
        if ($method == 'get') {
            ...
            throw new Error\PayhereError("Unrecognized method $method");
        }
    ...
    private function handleCurlError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            ...
            case CURLE_SSL_PEER_CERTIFICATE:
                $msg = "Could not verify Payhere's SSL certificate.  Please make sure "
    ...
    {
        if ($numRetries >= Payhere::getMaxNetworkRetries()) {
            return false;
        }
    ...
    private function sleepTime($numRetries)
    {
        ...
        $sleepSeconds = min(
            Payhere::getInitialNetworkRetryDelay() * 1.0 * pow(2, $numRetries - 1),
            Payhere::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = max(Payhere::getInitialNetworkRetryDelay(), $sleepSeconds);
    ...
    private function canSafelyUseHttp2()
    {
        ...
        // (cf. https://github.com/curl/curl/issues/2416), which Payhere use.
