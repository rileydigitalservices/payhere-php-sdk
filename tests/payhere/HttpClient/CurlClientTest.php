...
class CurlClientTest extends TestCase
{
    ...
    {
        $this->origMaxNetworkRetries = Payhere::getMaxNetworkRetries();
        $this->origMaxNetworkRetryDelay = Payhere::getMaxNetworkRetryDelay();
        $this->origInitialNetworkRetryDelay = Payhere::getInitialNetworkRetryDelay();
    }
    ...
    {
        $stripeReflector = new \ReflectionClass('\Payhere\Payhere');
        ...
        $curlClientReflector = new \ReflectionClass('Payhere\HttpClient\CurlClient');
    ...
    {
        Payhere::setMaxNetworkRetries($this->origMaxNetworkRetries);
    ...
    {
        $fakeRandomGenerator = $this->getMock('Payhere\Util\RandomGenetator', ['randFloat']);
    ...
    public function testDefaultOptions()
    {
        ...
        $this->setExpectedException('Payhere\Error\PayhereError', "Non-array value returned by defaultOptions CurlClient callback");
    ...
    {
        Payhere::setMaxNetworkRetries(2);
    ...
    public function testShouldNotRetryAtMaximumCount()
    {
        ...
        $this->assertFalse($this->shouldRetryMethod->invoke($curlClient, 0, 0, Payhere::getMaxNetworkRetries()));
    }
    ...
    public function testSleepTimeShouldGrowExponentially()
    {
        ...
        $this->assertEquals(
            Payhere::getInitialNetworkRetryDelay() * 1,
            $this->sleepTimeMethod->invoke($curlClient, 1)
        );
        $this->assertEquals(
            Payhere::getInitialNetworkRetryDelay() * 2,
            $this->sleepTimeMethod->invoke($curlClient, 2)
        );
        $this->assertEquals(
            Payhere::getInitialNetworkRetryDelay() * 4,
            $this->sleepTimeMethod->invoke($curlClient, 3)
        );
        $this->assertEquals(
            Payhere::getInitialNetworkRetryDelay() * 8,
    ...
    public function testSleepTimeShouldAddSomeRandomness()
    {
        ...
        $baseValue = Payhere::getInitialNetworkRetryDelay() * (0.5 * (1 + $randomValue));

        // the initial value cannot be smaller than the base,
        // so the randomness is ignored
        $this->assertEquals(Payhere::getInitialNetworkRetryDelay(), $this->sleepTimeMethod->invoke($curlClient, 1));
