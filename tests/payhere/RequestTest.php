...
class ApiRequestTest extends TestCase
{
    ...
    public function testDefaultHeaders()
    {
        ...
        Payhere::setAppInfo('MyTestApp', '1.2.34', 'https://mytestapp.example', 'partner_1234');
        $apiKey = 'sk_test_notarealkey';
        $clientInfo = ['httplib' => 'testlib 0.1.2'];

        $headers = $method->invoke(null, $apiKey, $clientInfo);

        $ua = json_decode($headers['X-Payhere-Client-User-Agent']);
        ...
        $this->assertSame(
            $headers['User-Agent'],
            'Payhere/v1 PhpBindings/' . Payhere::VERSION . ' MyTestApp/1.2.34 (https://mytestapp.example)'
        );
    ...
    {
        Payhere::setAccountId('acct_123');
        $this->stubRequest(
            'POST',
            '/v1/charges',
            [],
            [
                'Payhere-Account: acct_123',
            ],
            false,
            [
                'id' => 'ch_123',
